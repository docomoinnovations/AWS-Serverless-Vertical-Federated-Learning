import math, random, os, boto3, json, random
import torch
...
argparser.add_argument('-s', '--stack-name', type=str, help="Specify CloudFormation stack name of VFL")
...
for batch_index in tqdm(range(te_batch_count)):
    ...
    for i, client_model in enumerate(client_models):
        ...
        torch.save(client_embed.detach().cpu(), f'te_embed_{i+1}.pt')
    ...
    embed = ()
    for i in range(num_of_clients):
        e = torch.load(f'te_embed_{i+1}.pt')
        embed = (*embed, e)
