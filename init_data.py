...
TRAINING_DATA_SET_URL = 'https://archive.ics.uci.edu/ml/machine-learning-databases/adult/adult.data'
TEST_DATA_SET_URL = 'https://archive.ics.uci.edu/ml/machine-learning-databases/adult/adult.test'
...
def prepare_data():
    ...
    tr[lab_col] = tr[lab_col].replace({' <=50K': 0, ' >50K': 1}) # Be careful for white space
    va[lab_col] = va[lab_col].replace({' <=50K': 0, ' >50K': 1}) # Be careful for white space
    te[lab_col] = te[lab_col].replace({' <=50K': 0, ' >50K': 1}) # Be careful for white space
...
def get_client_cols(client):
    ...
    if client == 1:
        ...
        cols = [
            ...
            'workclass_3', 'workclass_4', 'workclass_5', 
            'workclass_6', 'workclass_7', 'workclass_8', 
        ...
        cols = [
            ...
            'education_9', 'education_10', 'education_11', 
...
if __name__ == '__main__':
    ...
    torch.save(tr_y, 'server/functions/server_training/tr_y.pt')
    torch.save(va_y, 'server/functions/server_training/va_y.pt')
    torch.save(te_y, 'test/te_y.pt')
    torch.save(tr_uid, 'server/functions/init_server/tr_uid.pt')
    torch.save(tr_uid, 'server/functions/server_training/tr_uid.pt')
    torch.save(va_uid, 'server/functions/init_server/va_uid.pt')
    torch.save(va_uid, 'server/functions/server_training/va_uid.pt')
    torch.save(te_uid, 'test/te_uid.pt')
    ...
    for client in range(1, 5):
        ...
        torch.save(tr_uid, f'client/dataset/client{client}/tr_uid.pt')
        torch.save(va_uid, f'client/dataset/client{client}/va_uid.pt')
        torch.save(client_tr_x, f'client/dataset/client{client}/tr_x.pt')
        torch.save(client_va_x, f'client/dataset/client{client}/va_x.pt')
        torch.save(client_cols, f'client/dataset/client{client}/cols.pt')
        torch.save(client_cols, f'test/cols_{client}.pt')
        torch.save(client_te_x, f'test/te_x_{client}.pt')
